...
module.exports = gql`
    ...
    """
    union TextMessageElement =
          ChatMessageTextElement
        | ChatMessageMentionElement
        | ChatMessageSymbolElement
    ...
    input CreateChannelInput {
        ...
        """
        Opt-in configuration option allowing any member who has the seed to join a Channel within the same Facility.
        Default behavior is only Members that already belong to the conversation being allowed to add
        new Members to the Channel.
        """
        openMembership: Boolean = false

        """
        Opt-in configuration option allowing members of the channel to mark individual messages as read.
        This will have an effect on the status calculation strategy and the ChatMessageConnection's metadata section.
        If the flag is set to false, the application will consider the high watermark approach.
        """
        individualReadReceipt: Boolean = false
    }
