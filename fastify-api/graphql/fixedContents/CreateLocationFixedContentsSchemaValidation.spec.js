const locationFixedContents = require('./CreateLocationFixedContentsSchemaValidation');

describe('Given we want to validate the Graphql schema to create a location`s fixed contents', () => {
    ...
    describe('when invalid schema is provided', () => {
        describe('and no locationId or fixedContent is provided', () => {
            it('then it should throw', async () => {
                await locationFixedContents.validateAsync({}, {abortEarly: false}).catch((err) => {
                    ...
                    expect(err.details[0].message).toBe('"fixedContent" is required');
                });
            });
        });
        ...
        describe('and incomplete fixedContent is provided', () => {
            it('then it should throw', async () => {
                await locationFixedContents
                    .validateAsync({locationId: 1, fixedContent: {}}, {abortEarly: false})
                    .catch((err) => {
