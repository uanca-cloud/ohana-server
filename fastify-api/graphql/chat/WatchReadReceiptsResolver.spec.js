...
beforeEach(() => {
    jest.mock('ohana-shared', () => ({
        ...
        getReadReceiptsAsyncIterator: jest.fn(),
        watchReadReceipt: jest.fn(),
        setChatReadReceiptsSubscriptionId: jest.fn(),
        getChatReadReceiptsSubscriptionId: jest.fn(),
        unWatchReadReceipt: jest.fn()
    }));

    resolver = require('./WatchReadReceiptsResolver');
...
describe('Given we want to resolve a GQL subscription to watch chat events', () => {
    ...
    describe('when we use the resolve function', () => {
        test('then it returns the async iterator for the connection', async () => {
            ohanaSharedPackage.getReadReceiptsAsyncIterator.mockImplementationOnce(function* () {
                yield true;
