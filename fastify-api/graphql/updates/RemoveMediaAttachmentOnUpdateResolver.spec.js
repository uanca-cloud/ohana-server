...
beforeEach(() => {
    jest.mock('ohana-shared', () => ({
        ...
        getMediaAttachment: jest.fn(() => ({type: 'photo'})),
        removalStrategies: jest.fn(() => true),
        writeLog: jest.fn(() => {})
    }));

    ohanaSharedPackage = require('ohana-shared');
    resolver = require('./RemoveMediaAttachmentOnUpdateResolver');
});

afterEach(() => {
    jest.unmock('./RemoveMediaAttachmentOnUpdateResolver');
    jest.unmock('ohana-shared');
});

describe('Given we want to resolve a GQL mutation to remove an media attachment from an update', () => {
