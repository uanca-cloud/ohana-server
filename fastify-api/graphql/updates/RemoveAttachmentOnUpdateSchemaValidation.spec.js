const removeAttachmentOnUpdate = require('./RemoveAttachmentOnUpdateSchemaValidation');

describe('Given we want to validate the Graphql schema for removing an attachment from an update mutation', () => {
    describe('when valid schema is provided', () => {
        it('then it should return the schema', async () => {
            const result = await removeAttachmentOnUpdate.validateAsync({
                input: {
                    encounterId: '1',
    ...
    describe('when invalid schema is provided', () => {
        describe('and no input is provided', () => {
            it('then it should throw', async () => {
                await removeAttachmentOnUpdate
                    .validateAsync({})
            ...
            it('then it should throw', async () => {
                await removeAttachmentOnUpdate
                    ...
                    .catch((err) => {
                        ...
                        expect(err.details[2].message).toBe('"input.id" is required');
                    });
            });
        });
        ...
        describe('and number is provided instead of string', () => {
            it('then it should throw', async () => {
                await removeAttachmentOnUpdate
                    ...
                    .catch((err) =>
                        expect(err.details[0].message).toBe('"input.id" must be a string')
                    );
            });
        });
    });
